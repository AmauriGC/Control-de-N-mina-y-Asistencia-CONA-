...
export const alertConfig = {
  success({ title = 'Éxito', text = '', timer = 1700, showConfirmButton = false } = {}) {
    return Swal.fire({ ...base, icon: 'success', iconColor: 'var(--color-primary)', title, text, timer, showConfirmButton })
  },
  error({ title = 'Error', text = '' } = {}) {
    return Swal.fire({ ...base, icon: 'error', iconColor: 'var(--color-destructive)', title, text, confirmButtonText: 'Entendido' })
  },
  info({ title = 'Información', text = '' } = {}) {
    return Swal.fire({ ...base, icon: 'info', iconColor: 'var(--color-ring)', title, text, confirmButtonText: 'Entendido' })
  },
  toast({
    title = '',
    text = '',
