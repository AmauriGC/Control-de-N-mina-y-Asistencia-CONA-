  ...
  return (
    <div className="p-8 space-y-6 min-h-screen">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Field label="Hora de Entrada" helper="Horario estándar de inicio" >
              <TimePicker value={config.workScheduleStart} onChange={(v) => handleConfigChange('workScheduleStart', v)} />
            </Field>
          ...
          <Field label="Tolerancia de Retardo (minutos)" helper="Minutos antes de marcar retardo">
            <Input type="number" value={config.lateThresholdMinutes} min={0} max={60} onChange={(e) => handleConfigChange('lateThresholdMinutes', parseInt(e.target.value))} />
          </Field>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Field label="Multiplicador Pago Vacacional" helper={`Veces el salario semanal (actualmente ${config.vacationPayMultiplier}x)`}>
              <Input type="number" min={1} max={5} value={config.vacationPayMultiplier} onChange={(e) => handleConfigChange('vacationPayMultiplier', parseInt(e.target.value))} />
            </Field>
            <Field label="Descuento por Vacaciones (MXN)" helper="Descuento aplicado al tomar vacaciones">
              <Input type="number" min={0} value={config.payrollDeduction} onChange={(e) => handleConfigChange('payrollDeduction', parseInt(e.target.value))} />
            </Field>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Field label="Plazo para Justificar (días)" helper="Días máximos para justificar">
              <Input type="number" min={1} max={7} value={config.justificationDeadlineDays} onChange={(e) => handleConfigChange('justificationDeadlineDays', parseInt(e.target.value))} />
            </Field>
            <Field label="Alerta de Contratos (días)" helper="Anticipación para contratos por vencer">
              <Input type="number" min={1} max={90} value={config.contractAlertDays} onChange={(e) => handleConfigChange('contractAlertDays', parseInt(e.target.value))} />
            </Field>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="flex items-center justify-between">
            ...
            <Dialog open={isHolidayDialogOpen} onOpenChange={setIsHolidayDialogOpen}>
              <DialogTrigger asChild>
                <Button className="gap-2"><Plus className="h-4 w-4" />Agregar Festivo</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                ...
                <HolidayForm onClose={() => setIsHolidayDialogOpen(false)} onAdd={(h) => { setHolidays((prev) => [...prev, h]); setIsHolidayDialogOpen(false) }} />
              </DialogContent>
            </Dialog>
          </div>
        </CardHeader>
            ...
            <TableBody>
              {holidays.map((holiday) => (
                <TableRow key={holiday.id}>
                  ...
                  <TableCell>{new Date(holiday.date).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</TableCell>
                  <TableCell>
                    <Badge variant={holiday.isRecurring ? 'default' : 'secondary'}>{holiday.isRecurring ? 'Recurrente' : 'Único'}</Badge>
                  </TableCell>
                  <TableCell>
                    <Button variant="ghost" size="sm" onClick={() => handleDeleteHoliday(holiday.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
...
function HolidayForm({ onClose, onAdd }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const newHoliday = { id: Date.now().toString(), name: nameField.value, date: dateField.value, isRecurring: formData.isRecurring }
    onAdd(newHoliday)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Field label="Nombre del Festivo *">
        <Input placeholder="Ej: Día de la Independencia" value={nameField.value} onChange={(e) => { setFormData({ ...formData, name: e.target.value }); nameField.onChange(e) }} onBlur={nameField.onBlur} aria-invalid={nameField.showError && !!nameField.error} />
        {nameField.showError && nameField.error && <p className="text-[12px] text-destructive">{nameField.error}</p>}
      </Field>
      <Field label="Fecha *">
        <Input type="date" value={dateField.value} onChange={(e) => { setFormData({ ...formData, date: e.target.value }); dateField.onChange(e) }} onBlur={dateField.onBlur} aria-invalid={dateField.showError && !!dateField.error} />
        {dateField.showError && dateField.error && <p className="text-[12px] text-destructive">{dateField.error}</p>}
      </Field>
      <div className="flex items-center gap-3">
        <input id="recurring" type="checkbox" checked={formData.isRecurring} onChange={(e) => setFormData({ ...formData, isRecurring: e.target.checked })} className="w-4 h-4 rounded border-input" />
        <Label htmlFor="recurring" className="cursor-pointer">Festivo recurrente (cada año)</Label>
      </div>
      <div className="flex justify-end gap-3 pt-4">
        <Button type="button" variant="outline" onClick={onClose}>Cancelar</Button>
        <Button type="submit" disabled={!nameField.isValid || !dateField.isValid}>Agregar Festivo</Button>
      </div>
    </form>
  )
}
